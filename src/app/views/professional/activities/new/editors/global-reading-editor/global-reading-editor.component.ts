...
import { GlobalReadingContent } from '../../../../../aac/activities/player/player.models';

@Component({
  selector: 'app-global-reading-editor',
  standalone: true,
  imports: [FormsModule, ColComponent, FormControlDirective, RowComponent, SpinnerComponent],
  templateUrl: './global-reading-editor.component.html',
  styleUrl: './global-reading-editor.component.scss',
})
export class GlobalReadingEditorComponent extends ContentEditorBaseComponent implements OnInit {
  private readonly arasaacService = inject(ArasaacService);

  content: GlobalReadingContent = { instruction: '', word: '', items: [], correctItemId: '' };
  ...
  ngOnInit(): void {
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        this.content = { instruction: '', word: '', items: [], correctItemId: '', ...parsed };
      }
    } catch { /* keep defaults */ }
  ...
  protected emit(): void {
    const valid = !!this.content.instruction.trim()
      && this.content.word.trim().length >= 2
