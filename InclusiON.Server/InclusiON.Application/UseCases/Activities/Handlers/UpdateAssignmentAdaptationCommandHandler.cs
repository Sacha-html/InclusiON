...
{
    public class UpdateAssignmentAdaptationCommandHandler
        : ICommandHandler<UpdateAssignmentAdaptationCommand, ApiResponse<ActivityAssignmentResponse>>
    {
        ...
        public UpdateAssignmentAdaptationCommandHandler(
        ...
        public async Task<ApiResponse<ActivityAssignmentResponse>> HandleAsync(
            UpdateAssignmentAdaptationCommand command, CancellationToken cancellationToken)
        {
            ...
            assignment.DueDate = command.DueDate;
            assignment.EstimatedDurationMinutes = command.EstimatedDurationMinutes;
            assignment.HasVisualSupport = command.HasVisualSupport;
            assignment.HasAudioSupport = command.HasAudioSupport;
            assignment.UsesEasyReading = command.UsesEasyReading;
            assignment.UsesPictograms = command.UsesPictograms;
            assignment.RequiresSupervision = command.RequiresSupervision;
            assignment.CustomAdaptationNotes = command.CustomAdaptationNotes;

            if (command.AcknowledgeAlert)
            {
            ...
            return ApiResponse<ActivityAssignmentResponse>.SuccessResult(dto, "Asignación adaptada exitosamente.");
        }
