...
namespace InclusiON.ApplicationBusiness.UseCases.Auth.Handlers
{
    ...
    public class ColorShapeLoginCommandHandler : ICommandHandler<ColorShapeLoginCommand, ApiResponse<VisualLoginResponse>>
    {
        ...
        private readonly ILogger<ColorShapeLoginCommandHandler> _logger;

        private const int MaxFailedAttempts = 5;

        public ColorShapeLoginCommandHandler(
            IVisualLoginRepository repository,
            UserManager<User> userManager,
            IJwtTokenService jwtTokenService,
            IRefreshTokensRepository refreshTokensRepository,
            IHttpContextAccessor httpContextAccessor,
            ILogger<ColorShapeLoginCommandHandler> logger)
        {
        ...
        public async Task<ApiResponse<VisualLoginResponse>> HandleAsync(
            ColorShapeLoginCommand command,
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (!person.ColorShapeId.HasValue)
                {
                    return ApiResponse<VisualLoginResponse>.ErrorResult("Color-forma no configurado");
                }

                if (command.ColorShapeId != person.ColorShapeId.Value)
                {
                    ...
                    return ApiResponse<VisualLoginResponse>.SuccessResult(
                        new VisualLoginResponse
                        {
                            ...
                            ErrorMessage = "Seleccion incorrecta"
                        });
                }
            ...
            {
                _logger.LogError(ex, "Error en login con color-forma para usuario: {UserId}", command.UserId);
            ...
            CancellationToken cancellationToken)
        {
            ...
            await _refreshTokensRepository.RevokeAllUserTokensAsync(user.Id, "Nuevo login con color-forma");
            ...
            if (rememberDevice && !string.IsNullOrEmpty(deviceId))
            {
                var device = new TrustedDevice
                {
                    UserId = user.Id,
                    DeviceId = deviceId,
                    DeviceName = "Dispositivo registrado via color-forma",
