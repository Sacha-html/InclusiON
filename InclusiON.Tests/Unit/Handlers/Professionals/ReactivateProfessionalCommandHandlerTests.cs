...
{
    public class ReactivateProfessionalCommandHandlerTests
    {
        ...
        private ReactivateProfessionalCommandHandler BuildSut() =>
            new(_prosRepo, _httpCtx, _uow,
                NullLogger<ReactivateProfessionalCommandHandler>.Instance, _dateTime);

        private static readonly Guid ProfId  = Guid.NewGuid();
        private static readonly Guid AdminId = Guid.NewGuid();

        private static ReactivateProfessionalCommand Cmd() =>
            new(ProfessionalId: ProfId, Observation: "Reactivación administrativa");

        private static Professional AProfessional(
            ProfessionalStatusEnum status = ProfessionalStatusEnum.Terminated,
            ...
            new()
            {
                ...
                UserId = Guid.NewGuid(),
                Status = status,
                User   = new User { IsActive = userActive },
        ...
        {
            _prosRepo.GetByIdAsync(ProfId, Arg.Any<CancellationToken>())
                     .Returns(AProfessional(ProfessionalStatusEnum.Approved, userActive: true));
        ...
        [Fact]
        public async Task HandleAsync_TerminatedProfessional_SetsApprovedAndActivatesUser()
        {
            ...
            professional.User.IsActive.Should().BeTrue();
