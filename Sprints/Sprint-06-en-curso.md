# Sprint 6 — En Curso

**Período:** Abril 2026

**Objetivo:** Migración de stack (.NET 10 + PostgreSQL), flujo de auto-registro profesional, documentación y mocks de actividades

---

## Tareas (14 ítems)

Leyenda: ✅ En Revisión · 🔄 En Curso · ⏳ Pendiente

| Código | Task | Tipo | Estado Jira |
|--------|------|------|-------------|
| IN-171 | [Backend] Migrar a PostgreSQL | Task | ✅ En Revisión |
| IN-147 | Migrar .NET 8 a .NET 10 | Story | ✅ En Revisión |
| IN-149 | Implementar flujo de auto-registro (Sign-up) para el rol Profesional | Story | ✅ En Revisión |
| IN-150 | Implementar flujo de selección institucional y validación por Administrador | Story | ✅ En Revisión |
| IN-148 | Implementar permiso de agrupación del núcleo familiar para el rol Profesional | Story | ✅ En Revisión |
| IN-165 | Crear HU Sprint 2 | Task | ✅ En Revisión |
| IN-166 | Crear HU's Sprint 3 | Task | ✅ En Revisión |
| IN-167 | Crear Historias de Usuario para Sprint 1 en Confluence | Task | ✅ En Revisión |
| IN-168 | Documentar procesos en Confluence | Task | ✅ En Revisión |
| IN-169 | Documentar WBS del proyecto | Task | ✅ En Revisión |
| IN-152 | Sprint y cuestionario (armar cuestionario para institución y familia) | Task | ✅ En Revisión |
| IN-170 | [Frontend] Revisar accesibilidad en componentes | Task | 🔄 En Curso |
| IN-151 | Tratar con Mauricio los requerimientos | Story | 🔄 En Curso |
| IN-163 | [Mock] Generar interfaz de alta de actividad | Task | ⏳ Pendiente |

---

## Resumen

| Métrica | Valor |
|---------|-------|
| Total tareas | 14 |
| En Revisión (For Review) | 12 |
| En Curso (In Progress) | 2 |
| Pendiente (To Do) | 1 |

---

## Backlog sin sprint asignado (próximos candidatos)

Items en To Do sin sprint — bloque más urgente según dependencias:

| Código | Task | Dependencia |
|--------|------|-------------|
| IN-163 | [Mock] Generar interfaz de alta de actividad | — (ya en Sprint 6) |
| IN-105 | Creación de actividad con wizard (área, template, contenido, metadatos) | Desbloquea todo |
| IN-106 | Integración de pictogramas ARASAAC | IN-105 |
| IN-107 | Consulta del catálogo de actividades | IN-105 |
| IN-108 | Edición de actividad propia | IN-105 |
| IN-109 | Desactivación de actividad | IN-105 |
| IN-110 | Creación del roadmap por persona | IN-105 |
| IN-111 | Agregar actividades al roadmap por área | IN-110 |
| IN-112 | Definir orden secuencial y umbral de desbloqueo | IN-110 |
| IN-113 | Reordenamiento drag-drop | IN-110 |
| IN-117 | Visualización del roadmap (vista estudiante, estilo Duolingo) | IN-110 |
| IN-86 | Timeline de diagnósticos en perfil de persona (vista admin) | BE-13 ✅ listo |
| IN-136 | Creación de reporte de progreso | BE-14 ✅ listo — cerrar en Jira |

---

## Épicas padre

- **IN-4:** Gestión de Usuarios
- **IN-6:** Autenticación y Accesibilidad
- **IN-9:** Gestión de Actividades ← próxima prioridad
- **IN-10:** Plan de Trabajo (Roadmap) ← próxima prioridad
